...
# Playing Pokemon Grow with Differentiable Cellular Automata

<div align="center">
<img src="/assets/ca/Wilson1900Fig2.jpg" width="100%">
<br>
From Wilson, Edmund B. The cell in Development and Inheritance (2nd ed.) 1900. PD from [Wikipedia](https://en.wikipedia.org/wiki/Cell_(biology)#/media/File:Wilson1900Fig2.jpg).
<br>
<br>
<br>
</div>
...
* They hold some promise as an avenue for a parsimonious learning machine. CA rulest optimized for a given task already offers some simplification over conventional deep networks that may have billions of parameters, but with the potential for training meta-learning CA we may find a generalizable set of learning rules that can operate on grids or graphs. Simpler models are also preferrable for having lower running costs.
...
I trained 3 separate models with 3 separate Pokemon, one for each perturbation. The result is 3 models each specialized to a given task and monster: growth (Pikachu), persistence (Charizard), and healing (Jigglypuff). If you are disappointed that I left out your favorite I am sorry, sort of, but you can always have a go at training based on the code in [the project repo](https://github.com/rivesunder/dca). 

### CA Model Formulation

As alluded to earlier, I implemented cellular automata rules as a series of two convolutional layers with single weight kernels. That formulation is in turn an implementation of applying the same dense multilayer perceptron model to each pixel. 
```
def update(perception, weights_0, weights_1, bias_0, bias_1):
    """ 
	returns a state_grid based on a computed neighborhood grid := perception
...
The Pokemon CA models described in this essay are each trained on a single sample image, so the concept of generalization doesn't really apply (yet). Training differentiable CA is tricky, I'd say somewhat more difficult than building and training your own GAN from scratch, but it's impressive when it works out. I expect to see development of differentiable CA for many of the toy and demo problems on images readily solvable by convolutional neural networks over the next year. Image processing will probably provide the first "low-hanging fruit," (as we've already seen with CA for classification and segmentation), but I predict we'll also see CA as a powerful tool for meta-learning in keeping with noted similarites to developmental biology. I'll be working on some of these problems myself, so stay tuned if that sounds like something you'd be interested in. Anyway, here's Wigglytuff trying to devolve back into Jigglypuff (at 8x speed) in a poignant metaphor of the saying "You can't go home again":
