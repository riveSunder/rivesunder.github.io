...
<img src="./assets/pikachu_comparison.png">
...
<img src="./assets/pikachu_explodes_px.gif">
...
We can try to mitigate malignant Pikachu by adjusting training to consider additional time steps and longer horizons when computing the loss function, although eventually we can expect to incur unnecessarily long training times and to eventually run out of memory from keeping track of all the gradients.

By sampling from several time steps after reconstructing the image, and also from many steps later, a more stable Pokemon can emerge. That's the difference in the training for the reasonable Snorlax below and the Pikagoo\*\* scenario. 

Now for the caveats. The Pokemon CA models described in this essay are each trained on a single sample, so they learn a very specific emergent developmental plan. Training differentiable CA is tricky, but it's impressive when it works out. Generalization across similar tasks with CA models remains to be thoroughly investigated. I expect we'll see development of differentiable CA for many of the toy and demo problems readily solvable by convolutional neural networks over the next year, and over time attain a better understanding of the capabilities and limitations of these models next to now conventional convolutional neural networks. Image processing will probably provide the first "low-hanging fruit," (as we've already seen with CA for classification and segmentation), but I predict we'll also see CA as a powerful tool for meta-learning in keeping with their noted similarites to developmental biology. I'll be working on some of these problems myself, so stay tuned if that sounds like something you'd be interested in. Anyway, here are more animations of Pokemon growing  out of control:

<img src="./assets/blooper_01.gif">
<br>
<img src="./assets/blooper_03.gif">
...
\*\* I'm only allowed to come up with one new joke a year and I hope you enjoyed this one! Or I am so, so sorry. 
